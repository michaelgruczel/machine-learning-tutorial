...
But in out case the image were perfectly scaled and positioned.
In a real world scenario the image might be not in perfect zoom and rotation.
So instead of mapping pixels we might want to identify patterns/features.
Therefore a CNN is a good fit.

We will use it here.
the algorithm is nearly the same with some small adaptions.
...
it comes to a precision of 93% for the training set and about 90% for the validation set:

```
60000/60000 [==============================] - 58s 960us/sample - loss: 1.5213 - accuracy: 0.9398

accuracy of our model against our test set: 0.9023
```
