...
public class MahoutResultEvaluation {
    ...
    public static void readFrequentPatterns(
        ...
        SequenceFile.Reader frequentPatternsReader = new SequenceFile.Reader(fs,
                new Path(fileName), configuration);
        Text key = new Text();
    ...
    public static void main(String args[]) throws Exception {
        ...
        String mappingCsvFilename = "./../data/MahoutAssociationLearningExampleMapping.csv";
        ...
        if (args.length != 6) {
            System.out.println("will use default args, if not use, Arguments: [transaction count] [mapping.csv path] [fList path] "
                    + "[frequentPatterns path] [minSupport] [minConfidence]");

        } else {
            transactionCount = Integer.parseInt(args[0]);
            mappingCsvFilename = args[1];
            frequencyFilename = args[2];
            frequentPatternsFilename = args[3];
            minSupport = Double.parseDouble(args[4]);
            minConfidence = Double.parseDouble(args[5]);
        }
